...
st.caption("Python-only Medallion pipeline outputs (CSV) — Gold Resolved + Gold Backlog")
...
def read_csv_or_empty(path: Path) -> pd.DataFrame:
    # Avoid pandas EmptyDataError and missing file issues
...
mask_people = st.sidebar.checkbox("🔒 Mascarar nomes (LGPD)", value=False)
...
if df_gold.empty and df_backlog.empty:
    st.warning(
        "Nenhum dataset Gold encontrado.\n\n"
        "Verifique se o pipeline gerou:\n"
        f"- {GOLD_RESOLVED_PATH}\n"
        f"- {GOLD_BACKLOG_PATH}\n"
    )
    st.stop()

# Apply LGPD masking (only affects dashboard view)
if mask_people:
...
selected_priority = st.sidebar.multiselect("Prioridade", options=priority_options, default=priority_options)
selected_type = st.sidebar.multiselect("Tipo do chamado", options=type_options, default=type_options)
selected_assignee = st.sidebar.selectbox("Analista", options=["All"] + assignee_options, index=0)
...
if not df_gold.empty:
    ...
    # ---- FIX: Streamlit slider requires min_value < max_value ----
    # If all values are the same (or dataset is very small), avoid crashing.
    if max_h_r <= min_h_r:
        sla_range = (min_h_r, max_h_r)
        st.sidebar.caption("Faixa SLA (horas úteis): todos os valores são iguais — filtro desabilitado.")
    else:
        sla_range = st.sidebar.slider(
            "Faixa SLA (horas úteis) — Resolved",
            min_value=min_h_r,
            max_value=max_h_r,
...
tab_overview, tab_reports, tab_viol, tab_dq, tab_backlog, tab_gov = st.tabs(
    [
        ...
        "📊 Relatórios (Gold Resolved)",
        "🚨 Violações (BREACHED)",
        "🧪 Data Quality (Silver)",
        "⏳ Backlog SLA (Gold Backlog)",
        "🏛️ Governança",
    ]
)
...
with tab_overview:
    ...
    if df_gold.empty:
        st.info("Gold Resolved dataset está vazio. Veja a aba Backlog para SLA em andamento.")
        st.stop()
    ...
    st.download_button(
        "⬇️ Baixar Gold Resolved filtrado (CSV)",
...
# ================================
with tab_reports:
    st.subheader("📊 Relatórios (Gold Resolved)")

    if df_gold.empty or df_view.empty:
        st.info("Sem dados Resolved para os filtros atuais.")
        st.stop()
    ...
    if ranking.empty:
        st.info("Sem registros MET para os filtros atuais.")
    else:
    ...
    st.divider()
    st.markdown("### 📋 Tabelas resumo (Gold Resolved filtrado)")
    ...
    with col_a:
        rep_analyst = (
            ...
            .agg(qtd_chamados=("issue_id", "count"), sla_medio_horas=("resolution_hours", "mean"))
            .reset_index()
            .sort_values("sla_medio_horas")
        )
        rep_analyst["sla_medio_horas"] = rep_analyst["sla_medio_horas"].round(2)
        st.caption("SLA médio por analista")
    ...
    with col_b:
        ...
        rep_type["sla_medio_horas"] = rep_type["sla_medio_horas"].round(2)
        st.caption("SLA médio por tipo do chamado")
...
# ================================
with tab_viol:
    st.subheader("🚨 Violações de SLA (BREACHED)")
    st.caption("Breach-only KPIs, analyst risk, and 'longest breaches' analysis (resolved issues only).")
    ...
    # If no breaches, show KPIs and stop
    if breached.empty:
        ...
        st.success("Nenhuma violação de SLA para os filtros atuais ✅")
        st.stop()

    # Analyst risk (breach % + volume)
    analyst_totals = (
    ...
    # Top Risk Analyst = highest breach_pct; tie-breakers: breached_count, then total_resolved
    ...
    # Risk charts by analyst (red)
    st.markdown("## 🧯 Breach Risk by Analyst")

    colA, colB = st.columns(2)

    with colA:
        st.markdown("### SLA Breached Ranking by Analyst (Top 20)")
    ...
    with colB:
        st.markdown("### SLA Breached Share by Analyst (Top 10)")
        pie_df = (
        ...
        others_count = int(
            risk.sort_values("sla_breached_count", ascending=False)["sla_breached_count"].iloc[10:].sum()
        ) if len(risk) > 10 else 0
        if others_count > 0:
            pie_df = pd.concat(
    ...
    # Breached tickets table + download
    st.download_button(
        "⬇️ Baixar violações (CSV)",
        data=to_csv_bytes(breached),
        file_name="sla_violacoes_breached.csv",
        mime="text/csv",
    )
    ...
    # Long breach charts BELOW the breached tickets list (this is what you asked for)
    st.divider()
    st.markdown("## 📈 Longest Breaches Analysis (below the breached tickets)")
...
with tab_dq:
    ...
    # Apply masking to invalids as well (dashboard-only)
    ...
    with col1:
        ...
        if dq_metrics_df.empty:
            st.info("silver_dq.json não encontrado (ou vazio). Rode a Silver normalize.")
        else:
            st.dataframe(dq_metrics_df, width="stretch")

        # DQ pie chart (valid / missing / invalid)
        ...
        if int(pie_counts["count"].sum()) > 0:
            ...
            st.info("Sem dados para o gráfico de DQ (contagens zeradas).")

    with col2:
        st.markdown("### 📄 Invalid records (Silver)")
        if invalid_df.empty:
            st.success("Nenhum inválido encontrado ✅")
        else:
            st.download_button(
                "⬇️ Baixar inválidos (CSV)",
...
# ================================
with tab_backlog:
    st.subheader("⏳ Backlog SLA (Gold Backlog)")
    st.caption("SLA age in business hours for open issues")

    if df_backlog.empty:
        st.info("Gold Backlog dataset está vazio. Rode o pipeline Gold backlog.")
        st.stop()
...
# ================================
with tab_gov:
    st.subheader("🏛️ Governança de Dados")
    ...
    with colA:
        ...
        if run_log.empty:
            st.info("data/audit/run_log.csv não encontrado. Rode o pipeline.")
        else:
    ...
    with colB:
        ...
        if not lineage:
            st.info("data/audit/lineage.json não encontrado. Rode o pipeline com lineage habilitado.")
        else:
            st.json(lineage)
    ...
    if dq_now:
        ...
        st.info("silver_dq.json não encontrado.")
